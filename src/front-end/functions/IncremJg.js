...
export default async function IncremJg(time, game) {
  ...
  function AddGameUser(jgd, user, tipoJG){
    ...
    } else if(tipoJG == "5x5Of"){
      console.log("5x5Of");
      user.scr5x5Of.jogos = user.scr5x5Of.jogos + 1;
      if(jgd.bool_dec)    user.scr5x5Of.vit = user.scr5x5Of.vit + 1;
      if(jgd.bool_cluth)  user.scr5x5Of.cluth = user.scr5x5Of.cluth + 1;
      user.scr5x5Of.total_pts = user.scr5x5Of.total_pts + ((jgd.a_2pts * 2) + (jgd.a_3pts * 3));
      user.scr5x5Of.a_2pts = user.scr5x5Of.a_2pts + jgd.a_2pts;
      user.scr5x5Of.a_3pts = user.scr5x5Of.a_3pts + jgd.a_3pts;
      user.scr5x5Of.reb = user.scr5x5Of.reb + jgd.reb;
      user.scr5x5Of.asst = user.scr5x5Of.asst + jgd.asst;
      user.scr5x5Of.block = user.scr5x5Of.block + jgd.blk;
      user.scr5x5Of.roubo = user.scr5x5Of.roubo + jgd.roubo;
      user.scr5x5Of.airB = user.scr5x5Of.airB + jgd.airB;
      // médias
      // calcula as médias 5x5 do jogador
      user.scr5x5Of.a_FG       = (user.scr5x5Of.vit * 100)  / user.scr5x5Of.jogos; // fg
      user.scr5x5Of.total_PPG  = user.scr5x5Of.total_pts    / user.scr5x5Of.jogos; // total ppg
      user.scr5x5Of.a_2PG      = user.scr5x5Of.a_2pts       / user.scr5x5Of.jogos; // 2pts por game
      user.scr5x5Of.a_3PG      = user.scr5x5Of.a_3pts       / user.scr5x5Of.jogos; // 3pts por game
      user.scr5x5Of.a_AirPG    = user.scr5x5Of.airB         / user.scr5x5Of.jogos; // airB por game
      user.scr5x5Of.a_RPG      = user.scr5x5Of.reb          / user.scr5x5Of.jogos; // reb por game
      user.scr5x5Of.a_APG      = user.scr5x5Of.asst         / user.scr5x5Of.jogos; // asst por game
      user.scr5x5Of.a_BPG      = user.scr5x5Of.block        / user.scr5x5Of.jogos; // block por game
      user.scr5x5Of.a_RouPG    = user.scr5x5Of.roubo        / user.scr5x5Of.jogos; // roubo por game
    }
    if(tipoJG != "5x5Of"){
  ...
  if(game.timeS.length > 0){ 
    for(let jgd of game.timeS){
      if(jgd.bool_jogou){ 
        jogo.timeS.push(jgd);
        AddGameUser(jgd, time.list_users[jgd.Users_idUsers], game.tipo_Jogo);
      }
    }  
  }
